...
def getGlueSite(site_name):
    site = None
    try:
        sites = gluesite.objects.filter(uniqueid = site_name)
        # use first one, it should be only one site
        site = sites[0]
    except:
        pass
        ...
        try:
            ...
            if se.uniqueid not in space_counts_se:
                space_counts_se[se.uniqueid] = {
                    'totalsize' : int(se.totalonlinesize) + int(se.totalnearlinesize),
                    'usedsize'  : int(se.usedonlinesize)  + int(se.usednearlinesize)}
            else:
                space_counts_se[se.uniqueid]['totalsize'] += int(se.totalonlinesize) + int(se.totalnearlinesize)
                space_counts_se[se.uniqueid]['usedsize']  += int(se.usedonlinesize)  + int(se.usednearlinesize)
...
def countStoragesInSite(site_entity):
    ...
    for se in se_list:
        if se.uniqueid in SPACE_COUNTS_SE:
            totalsize += SPACE_COUNTS_SE[se.uniqueid]['totalsize']
            usedsize  += SPACE_COUNTS_SE[se.uniqueid]['usedsize']
