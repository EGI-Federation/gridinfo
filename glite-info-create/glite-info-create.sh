...
function print_help() {
  cat << EOF >&2
Usage:
  glite-info-create.sh -m <module> [-i <iface>] [-t <templates>] [-c <config>]
...
for config_file in $config_files; do
  ...
  for template in $templates; do
    ...
    num=${line%%.cfg*}
    ldif_file=output/$module.$num.$template.ldif
    ldif_files="$ldif_files $ldif_file"
    if [ -f $ldif_file ]; then
      echo "WARNING: $ldif_file already existed. It has been overwritten."
    fi
