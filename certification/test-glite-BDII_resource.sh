#!/bin/bash
################################################################################
#                       Resource Level BDII Test Script
#
...
#  3 - Problem checking out resource level BDII tests from the Product Team
#  4 - Problems running the test-bdii-resource script
#
################################################################################
...
# Checkout latest resource level BDII tests from the Product Team
echo "INFO: Downloading top level BDII tests from the Product Team..."
svn co http://svnweb.cern.ch/guest/gridinfo/tests/trunk
if [ $? -ne 0 ]; then
  quit 3 "Problem checking out resource level BDII tests from the Product Team."
fi

# Running resource level BDII tests from the Product Team
echo "INFO: Running top level BDII tests from the Product Team..."
cd trunk
./test-bdii-resource
if [ $? -ne 0 ]; then
  quit 4 "Problems running the test-bdii-resource script."
fi
cd -
