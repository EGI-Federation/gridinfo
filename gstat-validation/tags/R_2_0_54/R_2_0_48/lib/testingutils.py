...
def parse_options():
    ...
    try:
        opts, args = getopt.getopt(sys.argv[1:], "t:H:p:b:f:d:o:t:sn",
          ["test", "host", "port", "bind", "file", "debug", "output",
          "summary", "timeout", "nagios"])
    ...
    if not (config.has_key('test')):
        sys.stderr.write("Error: Must specify a test module\n")
        usage()
        sys.exit(1)
...
Mandatory Parameters:
 -t --test      Specifies the testing module to use (E.g.: glue20)
